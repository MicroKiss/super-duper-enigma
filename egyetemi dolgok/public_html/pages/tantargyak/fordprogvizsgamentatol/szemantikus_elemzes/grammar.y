...
be:
    BE AZONOSITO
    {
        if( szimb_tabla.count( *$2 ) == 0 || szimb_tabla[*$2].vtip == Valos )
        {
            std::cerr << d_loc__.first_line << ": A(z) '" << *$2 << "' valtozo nincs deklaralva vagy valos szam." << std::endl;
            exit(1);
        }
        else
        {
...
ki:
    KI kifejezes
    {
		if( $2->ktip == Valos )
        {
            std::cerr <<"Valos szamokra nincs definiálva a KI muvelet." << std::endl;
            exit(1);
        }
        $$ = new utasitas_leiro( d_loc__.first_line );
        delete $2;
...
|
	TORT_RESZ BALZAROJEL kifejezes JOBBZAROJEL
    {
        if( $3->ktip != Valos )
        {
            std::cerr  << "Nem Valos kifejezés van megadva a tortresz fuggveny parameterenek." << std::endl;
            exit(1);
        }
		$$ = new kifejezes_leiro( d_loc__.first_line, Egesz );
        delete $3;
...
|
	EGESZ_RESZ BALZAROJEL kifejezes JOBBZAROJEL
    {
        if( $3->ktip != Valos )
        {
            std::cerr  << "Nem Valos kifejezés van megadva az egesz_resz fuggveny parameterenek." << std::endl;
            exit(1);
        }
		$$ = new kifejezes_leiro( d_loc__.first_line, Egesz );
        delete $3;
